...
async function runAudit() {
  ...
  /*
   * IMPORTANT:
   * This script uses date-only reconciliation.
   * The actual application's task overdue logic may use due_time/current
   * timestamp semantics.
   *
   * Therefore these numbers must be compared with the application's
   * documented business logic rather than blindly declared correct.
   */
  ...
  if (usersError) {
    ...
    (users || []).slice(0, 20).forEach(user => {
      console.log({
        id: user.id,
        name: user.name,
        role: user.role?.name || user.role || 'Unassigned',
      });
    });
  }
  ...
  console.log('\nIMPORTANT:');
  console.log(
    'These values establish database observations only.'
  );
  console.log(
    'They do NOT prove frontend correctness, RLS security, realtime behavior, or application KPI correctness.'
  );
  console.log(
